...
pub async fn chat(
    ...
    if is_stream {
        match proxy::proxy_stream(&client, engine_port, "/v1/chat/completions", Bytes::from(openai_body)).await {
            Ok(stream_body) => {
                Response::builder()
        ...
        match proxy::proxy_request(&client, engine_port, "/v1/chat/completions", Bytes::from(openai_body)).await {
            Ok((status, text)) => {
...
pub async fn tags(
    ...
    let models: Vec<serde_json::Value> = index.list().iter().map(|m| {
        serde_json::json!({
            "name": m.id,
            "model": m.id,
            "modified_at": null,
...
fn translate_openai_to_ollama_chat(openai_text: &str) -> String {
    ...
    let reasoning = openai["choices"][0]["message"]["reasoning_content"]
        .as_str();
